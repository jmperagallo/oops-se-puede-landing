...
export default function DashboardStats() {
  ...
  const cargarStats = async () => {
    ...
    try {
      // ✅ Usar supabaseAdmin para contar productos
      const { count: totalProductos, error: errorProductos } = await supabaseAdmin
        .from("productos")
        .select("*", { count: "exact", head: true });

      if (errorProductos) console.error("❌ Error contando productos:", errorProductos);

      // ✅ Usar supabaseAdmin para contar categorías
      const { count: totalCategorias, error: errorCategorias } = await supabaseAdmin
        .from("categorias")
        .select("*", { count: "exact", head: true });

      if (errorCategorias) console.error("❌ Error contando categorías:", errorCategorias);

      // ✅ Usar supabaseAdmin para contar visitas hoy
      const hoy = new Date();
      hoy.setHours(0, 0, 0, 0);
      const hoyISO = hoy.toISOString();

      const { count: visitasHoy, error: errorVisitasHoy } = await supabaseAdmin
        .from("visitas")
        .select("*", { count: "exact", head: true })
        .gte("fecha_visita", hoyISO);

      if (errorVisitasHoy) console.error("❌ Error contando visitas hoy:", errorVisitasHoy);

      // ✅ Usar supabaseAdmin para contar visitas totales
      const { count: visitasTotal, error: errorVisitasTotal } = await supabaseAdmin
        .from("visitas")
        .select("*", { count: "exact", head: true });

      if (errorVisitasTotal) console.error("❌ Error contando visitas totales:", errorVisitasTotal);
